...
def _normalize_channel_name(name):
    """
    Normalize channel name by removing stream variant suffixes so duplicates get merged.
    
    Catalog API uses letter suffixes:  "RTL .s", "RTL .b"  → "RTL"
    Index API uses numeric suffixes:   "RTL (1)", "RTL (2)" → "RTL"
    Both variants for the same channel get collapsed into one entry with multiple URLs.
    """
    normalized = name.strip()
    # Remove catalog-style suffixes: " .s", " .b", " .hd", etc.
    normalized = re.sub(r'\s+\.[a-zA-Z]+$', '', normalized)
    # Remove index-style numeric suffixes: " (1)", " (12)", etc.
...
def resolve_link(link, timeout=10):
    """
    ...
    """
    utils.log_debug(f"Resolving link: {link} (timeout={timeout}s)", "resolve_link")
    ...
    try:
        result = _make_request('post', url, silent=True, data=json.dumps(data), headers=headers, timeout=timeout)
...
def _collect_channels(group, channels):
    """
    Collect all channels for a group via the catalog API (handles pagination).
    Normalizes channel names to merge duplicates (.s/.b suffixes).
    """
    utils.log_debug(f"Collecting channels for group: {group}", "_collect_channels")
    cursor = 0
    total_collected = 0
    page_num = 0
...
def _get_index_data():
    """
    Fetch and cache the full index from INDEX_URL.
    The index is the authoritative source of all channels - it contains ALL entries
    with group, name (including (N) stream variants) and url.
    Returns a list of items, or [] on failure.

    Stream number meanings (from index.json analysis):
      (1) = server 1   (6) = server 6   (7) = server 7
    All are stripped by _normalize_channel_name and merged into one entry.
    """
    cache_key = "index_raw_data"
    cached = utils.get_cache(cache_key)
    if cached is not None:
        utils.log_debug(f"Returning cached index data ({len(cached)} items)", "_get_index_data")
        return cached
    try:
        utils.log_debug(f"Fetching raw index from: {INDEX_URL}", "_get_index_data")
        data = _make_request('get', INDEX_URL, params={"output": "json"}, timeout=10, silent=True)
        if isinstance(data, list):
            utils.set_cache(cache_key, data, COUNTRY_CACHE_TIMEOUT)
            utils.log_debug(f"Cached {len(data)} index items", "_get_index_data")
            return data
    except Exception as e:
        utils.log(f"Failed to fetch index data: {e}", "_get_index_data", force=True)
    return []


def _collect_channels_from_index(group, channels):
    """
    Supplement 'channels' with entries from the index for the given group.
    - Adds channels not found in catalog at all
    - Adds extra URLs to existing channels if not already present
    Catalog results take priority (already in dict), index fills the gaps.

    URLs from the index are sorted by server reliability before being added:
      Server (1) first  → 39% of channels, most coverage = most reliable
      Server (6) second → 38% of channels
      Server (7) last   → 23% of channels
    This ensures the most reliable server appears first in the stream list.

    NOTE: The index uses group names directly (Germany, France, etc.) - no Balkans
    DE-filter needed. Balkans in the index is purely Balkan content.
    """
    # Server priority map: lower number = listed first in stream selection
    SERVER_PRIORITY = {1: 0, 6: 1, 7: 2}

    utils.log_debug(f"Supplementing from index: group={group}", "_collect_channels_from_index")
    index_data = _get_index_data()

    # Collect all index entries for this group, tagged with server priority
    # Structure: {normalized_name: [(priority, url), ...]}
    pending = {}
    for item in index_data:
        ...
        # Extract server number from name suffix e.g. "RTL (6)" -> 6
        server_match = re.search(r'\((\d+)\)$', raw_name)
        server = int(server_match.group(1)) if server_match else 99
        priority = SERVER_PRIORITY.get(server, 99)

        normalized_name = _normalize_channel_name(raw_name)

        if normalized_name not in pending:
            pending[normalized_name] = []
        pending[normalized_name].append((priority, item_url))

    # Merge into channels dict in server-priority order
    added = 0
    for normalized_name, priority_urls in pending.items():
        # Sort by server priority so server 1 comes first
        ...
        for _, url in priority_urls:
            if url not in channels[normalized_name]:
                channels[normalized_name].append(url)

    utils.log_debug(f"Index supplement added {added} new channels for group: {group}", "_collect_channels_from_index")
...
def getchannels_by_group(group):
    ...
    _collect_channels_from_index(group, channels)
...
def refresh_country_cache():
    """Clear the country list cache and index cache so both re-fetch from API."""
    utils.log_debug("Clearing country + index cache", "refresh_country_cache")
    for cache_key in ("available_countries", "index_raw_data"):
        try:
...
def refresh_group_cache(group):
    ...
    Also clears the index cache so both sources deliver fresh data together.
    """
    global _channels_cache
    utils.log_debug(f"Clearing cache for group: {group}", "refresh_group_cache")
    for cache_key in (f"channels_group_{group}", "index_raw_data"):
        try:
            filepath = os.path.join(utils.cachepath, f"{cache_key}.json")
            if os.path.exists(filepath):
                os.remove(filepath)
...
def _select_stream(name, urls):
    ...
    # Specific stream selected (dialog index 0 = stream 0, etc.)
    stream_idx = dialog_index
...
def livePlay(name, urls=None, group=None):
    ...
    - If only one stream: plays it directly.
    - If multiple streams: shows selection dialog for the user to choose.
    ...
    progress.create('', f'Resolving {name}...')
    progress.update(50)
    ...
    try:
        ...
        utils.error_dialog("Stream Failed", "Stream failed to start.\nPlease use another stream or refresh the channel list and try again.")
        return
...
def _create_channel_listitem(name, url_count, group=None):
    ...
    plot = "[COLOR darkorange]TV Favorite[/COLOR]" if is_fav else ""
...
def favchannels():
    ...
    - Always handles empty state gracefully
    """
    ...
    if favorites:
        for fav in favorites:
            ...
            safe_name = quote_plus(real_name)
            ...
            # Use raw group (NOT safe_group) - convertPluginParams handles encoding internally
            items.append(({"name": real_name, "group": group}, listitem, False))
...
def change_favorit(name, group=None, delete=False):
    ...
    if delete:
        ...
        if not group:
            ...
            utils.notify("Failed to add favorite", heading="Error", icon=xbmcgui.NOTIFICATION_ERROR)
            return
...
def makem3u(group):
    ...
    if group == "favorites":
        # 1. Load Favorites
        progress = xbmcgui.DialogProgress()
        progress.create("", "Loading Favorites...")
        ...
        progress = xbmcgui.DialogProgress()
        progress.create("", f"Fetching channels for {group}...")
    ...
    # 4. Save File
    # Format: Oracle-GroupName-YYYY-MM-DD.m3u
    date_str = datetime.date.today().strftime("%Y-%m-%d")
    filename = f"Oracle-{clean_group}-{date_str}.m3u"
